...
public class SocialNetwork {
    ...
    public void createDataStructure(String str) {
		...
		if(null!=str){
			...
			for(String token : tokens){
				String[] sentence = token.split(" ");
				if(null!= sentence && null == getUser(sentence[0]))
				{
					User user = new User();
					user.setName(sentence[0]);
					String[] cons = sentence.length;
					user.setNumber(cons.length);
					for(i = 3; i < cons.length; i++) {
						
						user.setConnection(cons[i]);
					}
    ...
    public User[] getCommonConnections(User userA, User userB) {
		...
		if(userA.equals(userB)){
			...
			User[] common = null;
			for(User check : userA.getConnections()){
				if(userB.getConnections.contains(check)){
					common.add(check);
				}
			}
			return common;
